...
/**
 * 五种模式AI视频生成引擎设计对比实验
 * 使用DeepSeek API生成真实技术设计书
 * 团队：Shepherd/Agile/Waterfall/Chaotic/Spiral
 * 每个团队6名成员，5名专家打分
 */
...
// 技术设计书生成
async function generateTechDoc(teamName, mode, members) {
  console.log(`\n📋 ${teamName} - 生成技术设计书...`);
  ...
  for (const member of members) {
    ...
    const prompt = `[${teamName}模式]
你是${member.name}，${member.prompt}
...
请作为${member.name}，撰写以下技术设计书章节（300-500字）：
${getSectionTitle(member.role)}
...
直接输出方案内容，不要总结。`;
    ...
    try {
      ...
      sections.push({
        member: member.name,
        role: member.role,
        section: getSectionTitle(member.role),
        content: result,
        duration: duration
      });
...
function getSectionTitle(role) {
  ...
  return titles[role] || '技术方案';
}

// 专家评分
async function expertScore(expert, teamName, techDoc) {
  console.log(`   👨‍⚖️ ${expert.name} 评分...`);
  
  const docSummary = techDoc.map(s => 
    `## ${s.section}\n${s.content.substring(0, 300)}...`
  ).join('\n\n');
  
  const prompt = `你是${expert.name}，${expert.prompt}

请对以下${teamName}团队的技术设计书进行评分（0-100分）。
...
5. 与需求匹配度: XX - 理由`;
  ...
  try {
    ...
    return { expert: expert.name, scores, comment: result, duration };
  } catch (e) {
    console.log(`   ❌ ${expert.name} 失败: ${e.message}`);
    return { expert: expert.name, scores: { total: 0 }, comment: e.message, duration: 0 };
  }
}
...
// 生成完整技术设计书文档
function generateDocContent(teamName, mode, sections) {
  let content = `# AI视频生成引擎 - 技术设计书
...
async function runExperiment() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║     🎬 AI视频生成引擎 - 五种模式设计对比实验            ║');
  console.log('║     使用DeepSeek API生成真实技术设计书                   ║');
  console.log('╚══════════════════════════════════════════════════════════╝');
  console.log('');
  console.log('📋 实验配置：');
  console.log('   - 项目: AI视频生成引擎');
  console.log('   - 5个团队: Shepherd/Agile/Waterfall/Chaotic/Spiral');
  console.log('   - 每个团队: 6名成员（算法/架构/数据/前端/测试/PM）');
  console.log('   - 5名专家评委打分');
  console.log('   - 输出: 技术设计书 + 评分对比');
  console.log('');
  ...
  for (const mode of modes) {
    ...
    // 生成技术设计书
    const techDoc = await generateTechDoc(mode.name, mode, TEAM_MEMBERS);
    ...
    const docContent = generateDocContent(mode.name, mode, techDoc);
    ...
    for (const expert of EXPERTS) {
      const score = await expertScore(expert, mode.name, techDoc);
...
function generateComparisonReport(results) {
  ...
  console.log('📊 专家评分详情：');
  sorted.forEach((r, i) => {
    console.log(`\n${ranks[i]} ${r.mode} (平均: ${r.avgScore.toFixed(1)})`);
    r.expertScores.forEach(e => {
      const dims = e.scores;
      console.log(`   👨‍⚖️ ${e.expert}: ${dims.total}分`);
      if (dims['技术先进性']) console.log(`      技术先进性: ${dims['技术先进性']}/20`);
      if (dims['架构合理性']) console.log(`      架构合理性: ${dims['架构合理性']}/20`);
      if (dims['工程可行性']) console.log(`      工程可行性: ${dims['工程可行性']}/20`);
      if (dims['完整度']) console.log(`      完整度: ${dims['完整度']}/20`);
      if (dims['与需求匹配度']) console.log(`      与需求匹配度: ${dims['与需求匹配度']}/20`);
    });
  });
}
...
// 运行
async function main() {
  console.log('🎬 开始AI视频生成引擎设计实验...');
  console.log('⚠️  这将调用DeepSeek API，产生费用');
  console.log('   预计API调用: 50次（5团队×6成员 + 5团队×5专家）');
  console.log('');
